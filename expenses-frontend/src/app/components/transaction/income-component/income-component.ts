...
export class IncomeComponent implements OnInit {
  ...
  load() {
    ...
    this.tx.getTransactions().subscribe({
      next: data => {
        this.all = data;
        this.incomes = data.filter(x => x.type === 'INCOME');
        this.applyView();
        this.loading = false;
      },
      error: () => { this.errorMessage = 'Не вдалося завантажити доходи'; this.loading = false; }
    });
  }
    ...
    if (this.editing) {
      this.tx.updateTransaction(this.editing.id!, payload).subscribe({
        next: updated => {
          ...
          this.editing = null;
          Swal.fire('Успіх', 'Дохід оновлено', 'success');
        },
      ...
      this.tx.createTransaction(payload).subscribe({
        next: created => {
          ...
          this.applyView();
          this.modalVisible = false;
          Swal.fire('Успіх', 'Дохід додано', 'success');
        },
    ...
    Swal.fire({
      ...
      this.tx.delete(t.id!).subscribe({
        next: () => {
          ...
          this.incomes = this.all.filter(x => x.type === 'INCOME');
          this.applyView();
          Swal.fire('Готово', 'Дохід видалено', 'success');
        },
